...
async function extractInvoiceDetails(input: string): Promise<{
  ...
  try {
    ...
    const projectCodeMatch = input
      .trim()
      .match(/^\s*([A-Z]{2,}-[A-Z]+-\d+|[A-Z]{2,}\d+)\s*$/i);
    ...
    if (!result.period) {
      const now = new Date();
      result.period = `${String(now.getDate()).padStart(2, "0")}-${String(
        now.getMonth() + 1
    ...
    const projectCodeMatch = input.match(
      /\b(?:UP-[A-Z]+-\d+|PRJ-\d+|[A-Z]{2,}\d+)\b/i
    );
    ...
    const now = new Date();
    return {
      projectCode: projectCodeMatch
        ? projectCodeMatch[0].toUpperCase()
        : "UNKNOWN-PROJECT",
      clientName: "Client",
      period: `${String(now.getDate()).padStart(2, "0")}-${String(
        now.getMonth() + 1
      ).padStart(2, "0")}-${now.getFullYear()}`,
      items: [
        {
...
export async function parseWorkLogEntry(input: string): Promise<WorkLogEntry> {
  try {
    ...
    const clientMatch = input.match(
      /for\s+([A-Za-z0-9\s.&]+?)(?:\s+for|\s+on|\s+at|\s+$)/i
    );
...
export async function parseWithLLM(
  input: string
): Promise<Partial<ParsedInvoiceInput>> {
  ...
  try {
    ...
    const projectCodeMatch = input.match(
      /(?:project\s+)?([A-Za-z0-9][A-Za-z0-9-]{2,}(?:\s+[A-Za-z0-9-]+)*)/i
    );
    let projectCode = projectCodeMatch
      ? projectCodeMatch[1].trim().toUpperCase()
      : undefined;
    ...
    const periodMatch = input.match(
      /(?:for|period|month of|in)\s+(?:the\s+)?(?:month of\s+)?(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)(?:\s+(?:\d{4}|'?\d{2}))?/i
    );
    let period = periodMatch ? periodMatch[0] : undefined;

    // Extract transaction charges
    const transactionMatch = input.match(
      /(?:transaction\s*(?:fee|charge|cost)s?\s*(?:of)?\s*\$?\s*(\d+(?:\.\d{1,2})?)|\$?\s*(\d+(?:\.\d{1,2})?)\s*(?:dollars?|USD)?\s*(?:transaction\s*(?:fee|charge|cost)))/i
    );
    const transactionCharges = transactionMatch
      ? parseFloat(transactionMatch[1] || transactionMatch[2] || "0")
      : 0;
    ...
    const result: Partial<ParsedInvoiceInput> = {
      ...
      transactionCharges:
        transactionCharges || (parsed as any).transactionCharges || 0,
    ...
    if (result.projectCode && (!result.items || result.items.length === 0)) {
      const itemMatch = input.match(
        /(\d+)\s*(?:hours?|hrs?)\s*(?:of\s+)?([^\d$]+?)\s*(?:at\s*\$?\s*(\d+(?:\.\d{1,2})?))?/gi
      );
      if (itemMatch) {
        result.items = itemMatch.map((item) => {
          const [, quantity, desc, rate] =
            item.match(
              /(\d+)\s*(?:hours?|hrs?)\s*(?:of\s+)?([^\d$]+?)\s*(?:at\s*\$?\s*(\d+(?:\.\d{1,2})?))?/i
            ) || [];
          return {
...
export async function processWorkLog(
  input: string | ProcessWorkLogInput
): Promise<ProcessWorkLogResponse> {
  ...
  if (!isStringInput && input.action === "collect_email") {
    ...
    const emailInput = (
      typeof input.data?.email === "string" ? input.data.email : ""
    )
      .trim()
      .toLowerCase();
    ...
    if (
      emailInput.includes("download") ||
      emailInput.includes("link") ||
      emailInput.includes("don't send") ||
      emailInput.includes("no email")
    ) {
      return {
        action: "download_invoice",
        data: input.data || {},
    ...
    return {
      ...
      data: input.data || {},
      message:
        'That doesn\'t look like a valid email address. Please enter a valid email or type "download" to get a download link:',
    };
  }
  ...
  try {
    ...
    if (isStringInput) {
      ...
      if (
        !["1", "2", "3", "download", "email", "cancel", "no"].includes(
          lowerInput
        )
      ) {
    ...
    if (
      parsedInput.projectCode &&
      (!parsedInput.items || parsedInput.items.length === 0)
    ) {
      return {
        ...
        message:
          'I see you provided a project code. Could you please share the invoice details? For example: "5 hours of development at $75/hour, 3 hours of design at $60/hour for June 2023 with a $15 transaction fee"',
      };
    }

    if (parsedInput.items && parsedInput.items.length > 0) {
      const invoiceItems = (parsedInput.items || []).map(
    ...
    return {
      ...
      data: { input },
      message:
        'Please provide the project code and invoice details. Example: "Project ABC123, 5 hours of development at $75/hour for June 2023"',
    };
  } catch (error) {
    const errorMessage =
