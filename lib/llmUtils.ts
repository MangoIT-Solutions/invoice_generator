...
/**
 * Fetches project details from the database using project code
 */
async function getProjectDetails(projectCode: string): Promise<ProjectDetails | null> {
  try {
    const result = await client.execute({
      sql: 'SELECT client_name, client_email FROM projects_details WHERE project_code = ? LIMIT 1',
      args: [projectCode]
    });
