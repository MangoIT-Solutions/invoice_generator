...
export function generateInvoiceHtml(
  invoice: Partial<Invoice> & { [key: string]: any },
    ...
    <table class="main-table">
      <tr class="header-row">
        ...
        <td class="company-cell" colspan="2">
          ...
          <div style="font-size: 0.96em;">${company.contact} / ${
    company.email
  }</div>
      ...
      <tr>
        ...
        <td class="details-cell" colspan="2">
          <table class="details-table">
            ...
            <tr><td>Period:</td><td>${
              invoice.period ? formatPeriod(invoice.period) : "N/A"
            }</td></tr>
            <tr><td>Term:</td><td>${invoice.term || "On receipt"}</td></tr>
            <tr><td>Project code:</td><td>${
      ...
      <tr>
        <td colspan="3" class="ka-cell">KA: ${
          company.admin_name || "MangoIT Accounts"
        }<br />${company.admin_department || "Billing Department"}, ${
    company.name || "Mango IT Solutions"
  }</td>
      </tr>
      <tr>
        <td colspan="3" class="payment-instructions">Please wire as per bank details below & send SWIFT / bank advisory to ${
      ...
      <tr>
        <td colspan="3" style="padding:0;">
          <table class="bank-table">
            <tr><td class="bank-label">For credit to:</td><td>${
              company.name
            }</td></tr>
            <tr><td class="bank-label">Address:</td><td>${
              company.address
            }</td></tr>
            <tr><td class="bank-label">Account number:</td><td>${
              bank.account_number
            }</td></tr>
            <tr><td class="bank-label">Account with:</td><td>${
              bank.bank_name
            }, India</td></tr>
            <tr><td class="bank-label">Bank/Branch address:</td><td>${
              bank.bank_address
            }</td></tr>
            <tr><td class="bank-label">SWIFT:</td><td>${
              bank.swift_code
            }</td></tr>
            <tr><td class="bank-label">IFSC CODE:</td><td>${
              bank.ifsc_code
            }</td></tr>
